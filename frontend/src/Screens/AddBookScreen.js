...
const AddBookScreen = () => {
  ...
  const submit = async (inputData) => {
    ...
    if (edit === false) {
      res = await axios.get("http://localhost:5000/addbook", {
        params: inputData,
      });
    } else {
      res = await axios.get(
        `http://localhost:5000/updatebook/${location.state.college}${location.state.batch}${location.state.faculty}${location.state.rollno}`,
        { params: inputData }
      );
    }
